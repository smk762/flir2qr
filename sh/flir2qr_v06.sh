...
FLIR2QR_PATH="$(echo -e $(dirname ${2})";
ROADSFILE="$FLIR2QR_PATH/vector/TGDB_rds_4m_buff.shp";
GRIDFILE="$FLIR2QR_PATH/vector/100m_grid_wgs84_1m_buff.shp";
WILDFILE="${2%.*}";
MAPSIZE="2000 1600";
PIXELRANGE="0 8000";
timestamp=$(date +%F_%T);
IMG_PATH="$FLIR2QR_PATH/img";
echo -e "Started processing $2 at $timestamp" >> $FLIR2QR_PATH/log/flir2qr.log;
...
*.tif)
	...
	if [[ ("$i" -eq "10") ]]; then echo -e "$timestamp - $2 timed out waiting for world file $tfw" >> $FLIR2QR_PATH/log/error.log; exit 1; fi;
  ...
  echo -e "${GREEN}Dissolving IR clip fence $TEMP_PATH/clipfence/${WILDFILE}_fence_wgs84.shp${YELLOW}";
	...
	if [[ "$i" -eq "10" ]]; then echo "$timestamp - $2 Timed out waiting for KML" >> $FLIR2QR_PATH/log/error.log; exit 1; fi;
  ...
  gdal_translate $FLIR2QR_PATH/output/${WILDFILE}_basemap_wgs84.tif $FLIR2QR_PATH/output/${WILDFILE}.pdf -of PDF -a_srs EPSG:4326 -co EXTRA_IMAGES=$IMG_PATH/dpaw.jpg,0,0,1,$IMG_PATH/legend.jpg,5,45,1 -outsize $MAPSIZE;
  ...
  qrencode -o /var/www/html/hotspotmaps/qr/$WILDFILE.kml.png http://$IP/hotspotmaps/$WILDFILE.kml/doc.kml;
...
*)
  ...
  echo "$timestamp - bad filetype uploaded - $2" >> ${FLIR2QR_PATH}/log/error.log;
;;
esac

echo -e "$2 successfully processed!" >> $FLIR2QR_PATH/log/flir2qr.log;
echo -e "${BLUE}Processing $2 complete! ${WHITE}";
cp $FLIR2QR_PATH/log/* /var/www/html/hotspotmaps/log
